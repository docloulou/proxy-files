...
app.get('/url', verifyApiKey, (req, res) => {
  ...
  connectionsActives.set(connectionId, {
    url: req.query.url,
    baseUrl: req.query.url,     // URL de départ (celle passée en paramètre)
    bytesTransmis: 0,
    startTime: startTime,      // Dernière activité côté serveur (réception de données depuis la source)
    ...
    const remoteRequest = protocol.get(sourceUrl, options, remoteResponse => {
      ...
      remoteResponse.on('data', chunk => {
        ...
        if (connectionsActives.has(connectionId)) {
          ...
          connexion.url = sourceUrl; // Met à jour l'URL finale en cas de redirection
          // Mais on garde l'URL de base inchangée (baseUrl)
